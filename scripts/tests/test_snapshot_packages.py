#!/usr/bin/env python3
"""Exercise the snapshot gate's startup control flow without Docker or networking."""
import json
import os
...
HARNESS = ROOT / "scripts/test-snapshot-packages.sh"
...
args = sys.argv[1:]
events_path = Path(os.environ["SNAPSHOT_TEST_EVENTS"])
...
if command == "python3":
    if args and args[0] == "test-support/snapshot-packages/prepare.py":
        fixture = Path(args[1])
        (fixture / "repository").mkdir()
        for name in ("server.key", "ca.crt", "fixture.gpg"):
...
if args[0] == "exec":
    ...
    if args[2:] == ["cat", "/tmp/stats.json"]:
        print(json.dumps({"packageRequests": 4}))
        sys.exit(0)
    unexpected()
if args[0] == "inspect":
...
class SnapshotStartupTests(unittest.TestCase):
    def run_gate(self, mode):
        with tempfile.TemporaryDirectory(prefix="snapshot-harness-test-") as directory:
            ...
            stub.chmod(0o755)
            for name in ("docker", "python3", "sleep", "openssl", "gpg", "gpgconf", "dpkg-deb"):
            ...
            result = subprocess.run(
                ...
                env={**os.environ, "PATH": f"{commands}{os.pathsep}{os.environ['PATH']}",
                     "SNAPSHOT_TEST_EVENTS": str(events_path), "SNAPSHOT_TEST_MODE": mode},
    ...
    def assert_cleanup(self, events):
        ...
        self.assertTrue(any(name.startswith("openlegal-snapshot-client-") for name in cleanup[2:]))
    ...
    def assert_failed(self, mode, probes=1):
        ...
        self.assertTrue(any(event["kind"] == "docker" and event["args"][0] == "logs"
                            for event in events))
        self.assertNotIn("Snapshot package fixtures passed", result.stdout)
        return result, events
    ...
    def test_immediate_readiness_runs_client_and_statistics(self):
        ...
        self.assertIn("Snapshot package fixtures passed: baseline", result.stdout)
